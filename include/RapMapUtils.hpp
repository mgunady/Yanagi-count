	...
	*/
    ...
    struct QuasiAlignment {
  	QuasiAlignment() :
		...
		isPaired(false),
		tid2(std::numeric_limits<uint32_t>::max())
#ifdef RAPMAP_SALMON_SUPPORT
...
// Some convenience functions to allow salmon interop
	...
	// Adapted from
      ...
      inline int32_t mergeLeftRightSegments(
        ...
        inline MergeResult mergeLeftRightHitsSegmentsFuzzy(
                bool leftMatches,
                bool rightMatches,
                std::vector<QuasiAlignment>& leftHits,
                std::vector<QuasiAlignment>& rightHits,
          ...
          bool allowDovetail = mc.allowDovetail;
            if (leftHits.empty()) {
                ...
                if (rightHits.size() > 0) {
                    ...
                    auto rightLen = std::distance(rightIt, rightEnd);
                    size_t numHits{0};
                    jointHits.reserve(leftLen*rightLen);
//std::cerr << "hits: " << leftLen << " " << rightLen << "\n";

					for (auto& lh : leftHits) {
						for (auto& rh : rightHits) {
							auto segID1 = lh.tid;
							auto segID2 = rh.tid;
							std::vector<TranscriptIDType> interTxs;
							smap->transcriptsForPair(segID1, segID2, interTxs);
//std::cerr << "\nsgdshdjfdjdfjdjjsj " << segID1 << " " << segID2 << " " << interTxs.size() << " ";

							if(!interTxs.empty()) {
								auto posInTx1 = smap->getSegTxPosition(segID1, interTxs[0]);
								auto posInTx2 = smap->getSegTxPosition(segID2, interTxs[0]);
//std::cerr << "\n in condition " << posInTx1 << " " << posInTx2 << "\n";
								...
								// returned tuple is fwPos, rcPos, gapLength
								auto findBestHitFWRC = [signedZero, considerMultiPos, allowDovetail, &hadOppositeStrandMapping, 
											smap, segID1, segID2, interTxs, posInTx1, posInTx2](
                                                                       chobo::small_vector<int32_t>& fwdHits,
                                                                       chobo::small_vector<int32_t>& rcHits,
                                                                       int32_t fwdReadLen,
                                                                       bool& bestHitIsDovetail) ->
...
//std::cerr << "\n locs " << pos1 << " " << pos2 << "\n";
									...
									// positions that produced it.
									auto updateBestGap = [&bestGap, &bestFWPosIt, &bestRCPosIt,
														  signedZero, fwdReadLen, allowDovetail, pos1, pos2](
																					   chobo::small_vector<int32_t>::iterator fwdPosIt,
																					   chobo::small_vector<int32_t>::iterator rcPosIt
																					   ) {
									  // The gap between the end of the first read and the start of the
									  ...
									  int32_t gap = maxGap;
									  int32_t fwdPos = *fwdPosIt + pos1;
									  int32_t rcPos = *rcPosIt + pos2;
...
//std::cerr << "\n locs\n" << fwdPos << " " << rcPos << " " << gap << " " << bestGap << "\n";
									...
									for (auto pIt = fwdHits.begin(); pIt != fwdHits.end(); ++pIt) {
									  ...
									  // find the closest position for the rc read
									  auto lbIt = std::lower_bound(rcBeg, rcEnd, p1 + posInTx1 - posInTx2);
								...
								}; // End of findBestHitFWRC

                              // valid pairings have hits on opposite strands
                              auto& leftFwdHits = (lh.fwd) ? lh.allPositions : lh.oppositeStrandPositions;
                              auto& leftRCHits  = (lh.fwd) ? lh.oppositeStrandPositions : lh.allPositions;

                              auto& rightFwdHits = (rh.fwd) ? rh.allPositions : rh.oppositeStrandPositions;
                              auto& rightRCHits  = (rh.fwd) ? rh.oppositeStrandPositions : rh.allPositions;
...
//std::cerr << "\n bestMappingIsDovetail " << static_cast<bool>(bestMappingIsDovetail) << " " << static_cast<bool>(allowDovetail) << "\n\n";
                              ...
                              if (bestFWRC){
                                ...
                                leftFwd = true; rightFwd = false;
                                foundValidHit = true;
//std::cerr << "\n bestFWRC " << leftPos << " " << rightPos << " " << bestGap << "\n\n";
                              }
                              if (bestRCFW) {
                                int32_t fwPos, rcPos, bestGapRCFW;
                                std::tie(fwPos, rcPos, bestGapRCFW) = *bestRCFW;
                                ...
                                foundValidHit = true;
//std::cerr << "\n bestRCFW " << fwPos << " " << rcPos << " " << bestGapRCFW << "\n\n";
                              }
//std::cerr << "\n foundValidHit " << foundValidHit << "\n";
                              if (foundValidHit) {

                                // If we consider only a single position per transcript (XXX Segment)
                                int32_t startRead1 = std::max(leftPos, signedZero);
                                int32_t startRead2 = std::max(rightPos, signedZero);
                                bool read1First{(startRead1 < startRead2)};
                                int32_t fragStartPos = read1First ? startRead1 : startRead2;
                                int32_t fragEndPos = read1First ?
                                  (startRead2 + rightIt->readLen) : (startRead1 + lh.readLen);
                                uint32_t fragLen = fragEndPos - fragStartPos;								
                                jointHits.emplace_back(segID1,
                                                       leftPos,
                                                       leftFwd,
                                                       lh.readLen,
                                                       fragLen, true);
//std::cerr << "\n foundValidHit " << segID1 << " " << segID2 << "\n";
