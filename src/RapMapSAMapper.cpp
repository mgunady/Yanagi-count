...
void processReadsPairSA(paired_parser* parser,
    ...
    mc.consistentHits = false;//mopts->consistentHits;
    mc.doChaining = mopts->selAln;
    mc.allowDovetail = !mopts->noDovetail;
    if (mc.doChaining) {
      auto consensusSlack = mopts->consensusSlack;
      mc.consensusFraction = (consensusSlack == 0.0) ? 1.0 : (1.0 - consensusSlack);
      mc.considerMultiPos = true;
      ...
      for (auto& rpair : rg) {
        ...
        readLen = rpair.first.seq.length();
           ...
           if (useSmartIntersect) {
             ...
             if(isSegmentIndex) {
                mergeRes = rapmap::utils::mergeLeftRightHitsSegmentsFuzzy(
                                                                    lh, rh,
...
if(rpair.first.name.compare(str1)==0) {std::cerr << "\nread2449068/ENST00000369189\n"; mapTestFlags.print_flags(); std::cerr << "\n";}
             ...
             if ( mergeStatusOK and mopts->recoverOrphans and !tooManyHits) {
               if (leftHits.size() + rightHits.size() > 0) {
                 ...
                 //XXX TODO add segments support here
           ...
           if (mopts->selAln and !jointHits.empty()) {
             ...
             for (auto& h : jointHits) {
               ...
               if (h.mateStatus == rapmap::utils::MateStatus::PAIRED_END_PAIRED) {
                 ...
                 int32_t s2 =
                   selective_alignment::utils::getAlnScore(aligner, ez, h.matePos, r2ptr, l2, tseq2, tlen2, a, b, maxRightScore, h.chainStatus.getRight(),
...
//std::cerr << h.tid2 << " " << s2 << "\n";
                 ...
                 int32_t s =
                   selective_alignment::utils::getAlnScore(aligner, ez, h.pos, rptr, l2, tseq2, tlen2, a, b, maxRightScore, h.chainStatus.getRight(),
                                                           multiMapping, ap, buf, alnCacheRight);
...
//if(jointHits.size() == 151)
//    std::cerr << rpair.first.name << bestScore << '\n';
           } else if (noDovetail) {
             jointHits.erase(
                             std::remove_if(jointHits.begin(), jointHits.end(),
              ...
              if (jointHits.size() > 0 and jointHits.size() <= mopts->maxNumHits) {
                if(isSegmentIndex) {
                    ...
                    for(auto& h : jointHits) {
                        ...
                        // r2 -> lh : rc , r1 -> rh : rc = 4
                        uint8_t mappingType = h.fwd ? 2 : 0;
                        mappingType += h.mateIsFwd ? 1 : 0;
...
//	if(false) {
			...
			auto gene2 = smap->getGeneOfSeg(segID2);
                        ...
                        if(gene1 == gene2 && 
                           smallerPos > 0 && largerPos > 0 && 
                           lh.chainStatus.getLeft() == rapmap::utils::ChainStatus::PERFECT && rh.chainStatus.getRight() == rapmap::utils::ChainStatus::PERFECT) { // TODO: add dovetail and other necessary conditions
...
std::cerr << "\n" << numUnaligned << " pairs were unaligned\n";
    if (rmi.segInfo) {
